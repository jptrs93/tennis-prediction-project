...
refined joint optimisation time series Bradley-Terry model with 4 step, drift 0.9, and prior variance of 15. Runs the
model across 10 processes.
...
# Output file
output_file = "../../Outputs/Experiment06/RefinedJointOptBradleyTerry_4step_Prior15_drift0_9.csv"
...
def worker(q_in, q_out):
    ...
    """
    model = models.JointOptTimeSeriesModelRefined(steps=4,
                                                  optimiser=optimisers.JointOptTimeSeriesRefinedBradleyTerryVariationalInference(
                                                      prior_var=15., steps=4, drift=0.9, tol=1e-8, use_correlations=False))
    while 1:
        try:
